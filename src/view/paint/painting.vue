<template>
	<div>
		...
		<!-- 上传 -->
		<Drawer v-model="editShow" width="720" :mask-closable="true" :styles="styles" @on-close="closeDrawer">
			<task-detail :detail-data="editData"></task-detail>
		</Drawer>
		<!-- 预上画 -->
		<Drawer width="720" v-model="prePaintShow">
			<pre-painting ref="preData" :pre-detail-data="preDetailData" :pre-search-data="preSearchData"></pre-painting>
		</Drawer>
	</div>
</template>
<script>
	...
	export default {
								...
								h('Button', {
									...
									on: {
										click: () => {
											...
											this.$refs.taskData.getEditTitle('图片上传');
										}
									}
								}, '图片上传'),
								h('Button', {
		...
		methods: {
			//查询
			handleSearch() {
				//this.M_dateFormat(this.search.timeZoneBegin)
				let params = {
